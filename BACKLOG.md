...
## "Degenerate segue transitions" coercion silently bypasses the no-fade gate for any cart with a single-point (or absent) segue marker — likely the primary cause of widely-reported "songs cut short" behavior

**High priority** — found 2026-07-26, tracing an upstream Rivendell
v4.4.1 report (audio chopped off at segue points, e.g.
[ElvishArtisan/rivendell#1022](https://github.com/ElvishArtisan/rivendell/issues/1022))
back through this fork's own history to its actual origin.

**Root cause:** `RDLogPlay::StartEvent()` (`lib/rdlogplay.cpp`, top of
the "Transition running events" block) silently converts any `Segue`-
type transition whose `trans_length<=0` into a `Play`-type transition
before anything else runs. `RDLogLine::segueTail(RDLogLine::Segue)`
(`lib/rdlog_line.cpp:1541`) — the value passed as `trans_length` on the
ordinary segue-chain path (`RDLogPlay::segueStartData()`) — returns
`SEGUE_END_POINT - SEGUE_START_POINT`, which is exactly `0` for any cart
whose segue marker is a single point rather than a defined start/end
region. A `Play`-type transition's "Transition running events" branch
has **no gate of any kind** — it unconditionally stops every currently-
running deck, with no check of `segueGain()`/"no fade on segue out" at
all. The 2026-07-07 fix (see `ARCHITECTURE.md`'s "a flag gating one
side-effect of a compound action doesn't gate the others") added a
`segueGain()` gate to the `Segue` branch specifically — but this
coercion runs *before* that branch is ever reached, so it's a complete,
silent bypass of that protection for any cart hitting this condition.

**Historical origin, confirmed via `v4.1.3..v4.4.1` bisection of
upstream `ElvishArtisan/rivendell`:** the coercion was added in
`254b9bdc` (2024-08-25, Fred Gleason, "Fixed a regression... that could
cause events with Segue transitions to clip off the end of the previous
event if the previous event did not have a segue marker set") —
explicitly intended to fix a *different*, already-existing clipping bug
for carts with **no** segue marker at all, but written as a blanket
`trans_length<=0` check that also silently catches any cart with a
**legitimately configured, deliberately single-point** segue marker,
treating both cases identically. Confirmed still present, unchanged, in
this fork.

**Why this is probably bigger than any Marker-specific trigger
condition:** this path requires nothing unusual — no `Marker`-type log
line, no "Timed, Make Next," no special configuration at all. Any
ordinary song-to-song segue where the outgoing song's segue marker
happens to be a single point instead of a defined region hits this.
Brandon's own library always sets a 3000ms segue start/end window on
virtually every element, so this never triggers for him — but he
explicitly does not know whether that's true of other stations',
especially older libraries migrated over many years with less
consistent marker maintenance. A data-dependent trigger condition tied
to specific cuts' marker values (rather than a universal code path
everyone hits identically) would explain why the upstream community
spent months on this ([#1022](https://github.com/ElvishArtisan/rivendell/issues/1022),
124 comments, several contributor branches) without ever landing on one
clean, confirmed fix.

**Why this has not been fixed here yet:** changing `254b9bdc`'s coercion
requires first understanding what it was actually protecting against —
specifically, what `RDPlayDeck::stop()` does with a zero or very short
length, and what was clipping in the "no segue marker at all" case
*before* this coercion existed. Redesigning core segue-transition-type
logic without being sure what the original fix was protecting against
risks reintroducing an old bug while fixing this one. Needs its own
dedicated investigation, starting with `RDPlayDeck::stop()`'s
implementation, before any fix is designed.

**Related but distinct, already planned separately:** a Marker-specific
variant of this same general class of bug (a `RDLogLine::Type::Marker`
line, reached via Segue, synchronously cascading through
`RDLogPlay::FinishEvent()`'s hardcoded, equally ungated `Play`-type
auto-advance) has its own fix already designed — see the plan file from
this session (`fix-marker-segue-hard-stop-cascade`, not yet branched).
That fix does not address this entry at all — they are different call
sites into the same underlying class of problem (`segueStartData()`'s
direct `StartEvent()` call here, versus `FinishEvent()`'s hardcoded call
there) and neither substitutes for the other.

**Current mitigation:** none identified. Given Brandon's own operational
practice (a consistent, deliberate segue window on every cut), the most
likely practical mitigation for any station hitting this is auditing
the library for cuts whose `SEGUE_START_POINT`/`SEGUE_END_POINT` are
equal (or unset) and giving them a real window — not yet confirmed as a
complete workaround, since the "no marker at all" case still legitimately
needs *some* fallback behavior (see `RDPlayDeck::StartTimers()`'s
"Play Style Segue" branch).

**Deferred for now** — root cause identified and documented above, not
yet investigated deeply enough to design a fix.
