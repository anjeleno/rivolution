...
**Date:** 2026-06-30
**Revised:** 2026-07-02
**Revised again, 2026-07-10:** Liquidsoap has been replaced by a
per-stream `ffmpeg` process; see `docs/specs/
0015-ffmpeg-broadcast-output.md` for the full replacement and for
Stereo Tool's corrected JACK-routing mechanism. Every `liquidsoap.*`
unit reference below is historical (Phase 1 as originally shipped);
each stream now gets its own systemd unit generated by
`rivapi/store/ffmpeg_generator.go` instead of one shared
`liquidsoap.service`. The ordering/readiness/live-playout-protection
*design* described below still applies unchanged — only the concrete
unit names differ. `~/.asoundrc` (`rd.asoundrc` below) is also no
longer purely static: `syncStereoToolTarget`/`PatchAsoundrcTarget`
(`rivapi/store/stereo_tool_install.go`) rewrite its `playback_ports`
block automatically at deploy time to track whichever stream is
currently the dynamically-selected anchor.
...
Historical (as originally shipped — see the 2026-07-10 revision note
...
Current chain: `rivendell.service` -> Stereo Tool (its `.asoundrc`
target is patched to whichever stream is the dynamically-selected
anchor) -> each stream's own ffmpeg unit -> Icecast. Stereo Tool now
precedes the stream units rather than following a single shared
Liquidsoap unit; see `docs/specs/0015-ffmpeg-broadcast-output.md`.
...
Once `caed` is extracted into its own unit (`liquidsoap.service` below
is historical — see the 2026-07-10 revision note at the top of this
spec):
...
- **Each stream's ffmpeg unit** (historically `liquidsoap.service`;
  see `docs/specs/0015-ffmpeg-broadcast-output.md`): verify or add a
  readiness probe.
...
- Icecast mount/metadata/password changes: restart `icecast2.service`
  only. Each stream's ffmpeg process reconnects automatically after
  Icecast restarts.
- A single stream's codec/bitrate change: restart that stream's own
  unit only, other streams unaffected (historically: a Liquidsoap
  script-only change restarting `liquidsoap.service` alone; see
  `docs/specs/0015-ffmpeg-broadcast-output.md`).
...
external services (Icecast, a single stream's ffmpeg unit) are
restarted in isolation, RDAirPlay is unaffected.
...
- New: `/etc/systemd/system/rivolution-stack.target` — groups the full
  ...
  `stereo-tool.service`, `tailscaled.service`, and (see deviations
  below) `pipewire-system.service`/`wireplumber-system.service`, plus
  one generated unit per configured broadcast stream (replacing the
  single `liquidsoap.service` this list originally had — see the
  2026-07-10 revision note at the top of this spec).
...
- Historical (see the 2026-07-10 revision note at the top of this
  spec): `/etc/systemd/system/liquidsoap.service` — full standalone
  unit, not a drop-in (see deviations below): `ExecStart`,
  `Restart=on-failure`, `RestartSec=5`,
  `Environment=XDG_RUNTIME_DIR=/run/pipewire-system`.
- Historical: `/etc/systemd/system/liquidsoap.service.d/rivolution.conf`
  — drop-in adding `After=icecast2.service`, `After=rivendell.service`
  (the `[Unit]` ordering stanzas only; `ExecStart` lives in the full
  unit above, not here). Both of the above are replaced by one
  generated systemd unit per configured stream
  (`rivapi/store/ffmpeg_generator.go`, see `docs/specs/
  0015-ffmpeg-broadcast-output.md`).
- New: `/etc/systemd/system/stereo-tool.service` — custom unit for
  Stereo Tool; `User=rd`;
  `Environment=XDG_RUNTIME_DIR=/run/pipewire-system`; `-p 8079` on
  `ExecStart` to expose its headless web config UI. (Its `After=` no
  longer names `liquidsoap.service` — see `docs/specs/
  0015-ffmpeg-broadcast-output.md`.)
- New: `/etc/sudoers.d/rivapi` — targeted rule: `rd` may run
  `systemctl start/stop/restart` for `rivolution-stack.target`,
  `rivendell.service`, `icecast2.service`, `stereo-tool.service`,
  `tailscaled.service`, `pipewire-system.service`,
  `wireplumber-system.service`, and each generated stream unit, with
  `NOPASSWD`, plus `RIVAPI_INSTALL`
...
The `liquidsoap.service*` copies below are historical (see the
2026-07-10 revision note at the top of this spec) — a current install
has no fixed set of stream-unit files to copy at all, since one is
generated per configured stream at `/broadcast` deploy time.
...
After first boot, configure at least one stream on `/broadcast` and
Save & Deploy — this now drives the `caed` → Stereo Tool → stream
chain automatically (dynamic anchor selection + reconciler fan-out,
see `docs/specs/0015-ffmpeg-broadcast-output.md`), with `/patchbay`
available for manual inspection or override if needed.
...
`Depends`: `icecast2`, `ffmpeg` (replaces `liquidsoap` — see `docs/specs/
0015-ffmpeg-broadcast-output.md`), `fdkaac`, `vlc`, `vlc-plugin-jack`,
...
1. Copy every unit/drop-in listed under "Files" above
   ...
   `wireplumber-system.service`, `stereo-tool.service`,
   `rivapi.service`) to `/etc/systemd/system/` — stream units are
   generated per-configuration by `rivapi` itself, not shipped by the
   package (see `docs/specs/0015-ffmpeg-broadcast-output.md`).
...
8. Deploy `conf/alsa/rd.asoundrc` to the `rd` user's `~/.asoundrc`
   ...
   *connections between existing ports*; `rd.asoundrc`'s job is
   ...
   on a real install 2026-07-02. **Confirmed 2026-07-10: this file's
   `pcm.jack` `playback_ports` block is what Stereo Tool's "Normal
   output" device actually targets** (not `~/.stereo_tool.rc`'s own
   Jack ID fields, previously believed to be the mechanism); once
   deployed, `syncStereoToolTarget` (`rivapi/store/
   stereo_tool_install.go`) keeps that block's target current
   automatically on every `/broadcast` deploy — see `docs/specs/
   0015-ffmpeg-broadcast-output.md`.
