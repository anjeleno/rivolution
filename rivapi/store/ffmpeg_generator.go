...
// service per stream. See the liquidsoap-jack-investigation branch's
// patches/liquidsoap/README.md (kept as reference, never merged) and
...
// "JACK input ID" field, cfg.FfmpegOutput.JackInputID). A single client
// name can't be shared across N independent ffmpeg processes, so each
// stream's mount is appended to keep every client name distinct while
// still honoring what the operator configured as the station's base
// identity.
...
func ffmpegPipeline(cfg BroadcastConfig, s StreamConfig) (string, error) {
	clientID := streamJackClientID(cfg.FfmpegOutput.JackInputID, s.Mount)
	rate := cfg.FfmpegOutput.SampleRate
...
func icecastURL(cfg BroadcastConfig, s StreamConfig) string {
	return fmt.Sprintf("icecast://source:%s@%s:%d%s",
		cfg.Icecast.SourcePassword, cfg.FfmpegOutput.IcecastHost, cfg.FfmpegOutput.IcecastPort, s.Mount)
}
...
func DeployFfmpegStreams(cfg BroadcastConfig) error {
	...
	if cfg.FfmpegOutput.LogPath != "" {
		if err := os.MkdirAll(filepath.Dir(cfg.FfmpegOutput.LogPath), 0755); err != nil {
	...
	for _, s := range cfg.Streams {
		...
		if cfg.FfmpegOutput.LogPath != "" {
			logDirectives = fmt.Sprintf("StandardOutput=append:%s\nStandardError=append:%s\n",
				cfg.FfmpegOutput.LogPath, cfg.FfmpegOutput.LogPath)
		}
...
func syncStreamPatchLinks(cfg BroadcastConfig, previous, current map[string]string) error {
	...
	for mount := range previous {
		if _, stillPresent := current[mount]; !stillPresent {
			removedClients[streamJackClientID(cfg.FfmpegOutput.JackInputID, mount)] = true
		}
	}
